  ...
  try {
    ...
    box.innerHTML = list.length ? `<div class="grid" style="grid-template-columns:repeat(auto-fill,minmax(280px,1fr))">${sorted.map((p) => {
      ...
      <div class="card" style="padding:18px">
        ...
        ${p.winner ? `<div class="panel gold" style="margin-bottom:12px;padding:10px 12px"><div class="row-between" style="gap:8px"><span>🏆 Výherce: <b>${esc(p.winner)}</b></span><button class="btn btn-ghost btn-sm" data-action="raffle-undo" data-id="${p.id}" title="Smazat výherce – účastníci zůstanou">↩️ Vrátit</button></div></div>` : ""}
