...
async function apiRequest(path, opts = {}) {
  ...
  if (data && typeof data === "object" && !Array.isArray(data))
    Object.defineProperty(data, "_serverTime", { value: res.headers.get("date"), enumerable: false });
  return data;
}
...
function renderBannedPage() {
  ...
  const timeoutReason = (state.user && state.user.timeout_reason) || "Neuveden";
  ...
  const detail = timeout
    ? (Number.isFinite(until)
      ? `<strong>Zbývá:</strong> <span id="banTimeoutLeft"></span> · do ${esc(new Date(until).toLocaleString("cs-CZ"))}<br><strong>Důvod:</strong> ${esc(timeoutReason)}`
      : `${esc(timeout)}<br><strong>Důvod:</strong> ${esc(timeoutReason)}`)
...
function selfExcludeBlock(u) {
  ...
  return `<p class="muted" style="font-size:13px;line-height:1.6">Můžeš se dobrovolně zamknout ze <b style="color:var(--text)">všech sázek</b> (duely, piškvorky, blackjack, predikce). <b>Nejde to zrušit dřív</b>, tak si to rozmysli. Body ti zůstanou, jen nebudeš moct sázet.</p>
    <div class="se-btns">
...
function pageProfile() {
  ...
  const u = state.user;
  const view = $("#view");
  const pct = Math.max(0, Math.min(100, Number(u.level_pct || 0)));
  const ringOff = Math.round(302 - 302 * pct / 100);           // obvod kruhu r=48 ≈ 302
  const xpInto = Number(u.level_into || 0).toLocaleString("cs-CZ"), xpSpan = Number(u.level_span || 0).toLocaleString("cs-CZ");
  view.innerHTML = `
    <div class="page-head"><h1>👤 Můj profil</h1></div>
    <div class="prof-hero">
      <div class="prof-ring">
        <svg viewBox="0 0 110 110" aria-hidden="true"><defs><linearGradient id="profRingG" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#f7bd44"/><stop offset="1" stop-color="#d97a14"/></linearGradient></defs>
          <circle class="rbg" cx="55" cy="55" r="48"/><circle class="rfg" cx="55" cy="55" r="48" style="stroke-dashoffset:${ringOff}"/></svg>
        ${avatarHTML(u.username, u.avatar_url, "prof-ava", cosF(u))}
        ${u.level ? `<div class="prof-lvl-tag">⭐ Level ${u.level}</div>` : ""}
      </div>
      <div class="prof-id">
        <div class="prof-name"><span class="nick ${cosN(u)}">${esc(u.username)}</span> ${roleBadge(u.role)} ${subVipBadges(u)} ${prestigeBadge(u.prestige)}</div>
        <div class="prof-meta">${u.kick_username ? `<span>🟢 ${esc(u.kick_username)}</span>` : (u.email ? `<span>${esc(u.email)}</span>` : "")}<span title="Tvoje ID účtu">🆔 ID ${u.id}</span><span>🎂 V komunitě <b>${memberSince(u.created_at)}</b>${loyaltyBadge(u.created_at)}</span></div>
        ${u.level ? `<div class="prof-xpline" title="Úroveň z nafarmeného (earned_total)"><div class="lbl"><span>Level ${u.level} → ${u.level + 1}</span><b>${xpInto} / ${xpSpan} XP</b></div><div class="xpbar"><i style="width:${pct}%"></i></div></div>` : ""}
      </div>
      <div class="prof-bal">
        <div class="v">${fmtPts(u.points)} 🌾</div><div class="l">aktuální zůstatek sedláků</div>
        <div class="acts"><a href="#/vs" class="btn btn-ghost btn-sm">⚔️ Porovnat</a><button class="btn btn-accent btn-sm" data-action="my-wrapped">🎁 Moje čísla</button></div>
      </div>
    </div>
    <div class="prof-tiles">
      ${u.rank && userTier(u.rank) ? `<div class="prof-tile gold"><span class="ic">🏅</span><span class="v">★ ${userTier(u.rank)}</span><span class="l">tvoje liga</span></div>` : ""}
      <div class="prof-tile hot" id="ptStreak" hidden><span class="ic">🔥</span><span class="v"></span><span class="l">denní série</span></div>
      <div class="prof-tile gold" id="ptEarned" hidden><span class="ic">💰</span><span class="v"></span><span class="l">celkem nafarmeno</span></div>
      <div class="prof-tile" id="ptBadges" hidden><span class="ic">🎖️</span><span class="v"></span><span class="l">odznaky</span></div>
    </div>
    <div class="prof-cols">
      <div class="panel prof-main">
        <div id="myBio" style="margin:0 0 4px">${skeletonCards(1)}</div>
      ...
      <div class="prof-side">
        <div class="panel prof-side-card">
          <span class="ic">🎨</span>
          <span class="t"><b>Můj vzhled</b><span>barvy nicku a rámečky — vidí všichni</span></span>
          <a href="#/kosmetika" class="btn btn-accent btn-sm">Upravit →</a>
        </div>
        <div id="myPrestige"></div>
        <div class="panel">
          <div class="section-title" style="margin-top:0">🎁 Steam trade link</div>
          <p class="muted" style="margin:2px 0 12px;font-size:12.5px;line-height:1.55">
            Podle něj ti streamer pošle vyhrané skiny. Najdeš ho na Steamu →
            <b>Inventory</b> → <b>Trade Offers</b> → <b>„Who can send me Trade Offers?”</b>.
          </p>
          <div class="field">
          ...
          <div class="row-between" style="margin-top:10px;gap:10px;flex-wrap:wrap">
            ...
            <button class="btn btn-primary btn-sm" data-action="save-trade">💾 Uložit</button>
          </div>
          <details class="prof-safe">
            <summary>🔒 Zodpovědné sázení</summary>
            <div class="inner">
              ${selfExcludeBlock(u)}
...
async function loadProfTiles() {
  // dlaždice ze stávajících endpointů — žádný nový backend
  const show = (id, val) => { const el = document.getElementById(id); if (el && val) { el.querySelector(".v").textContent = val; el.hidden = false; } };
  try {
    const c = await api("/me/claims");
    if (c && c.streak > 0) show("ptStreak", c.streak + ". den");
  } catch (e) { /* dlaždice jsou bonus, profil bez nich žije */ }
  try {
    const p = await api("/profile/public?nick=" + encodeURIComponent(state.user.username));
    if (p) {
      show("ptEarned", fmtPts(p.earned_total || 0));
      const n = (p.badges || []).length;
      if (n) show("ptBadges", String(n));
    }
  } catch (e) { /* dtto */ }
}
...
function auctionCardHTML(a) {
  ...
  const bg = a.image_url ? `background-image:url('${esc(a.image_url)}');background-size:${a.seller ? "contain" : "100% 100%"}` : "";
...
async function adminCrews() {                              // admin přehled part: kdo s kým, level, XP, příspěvky
  ...
  const full = list[0].level != null;   // broadcaster dostává ze serveru ořezaná data (bez ekonomiky)
  box.innerHTML = `
    <p class="muted" style="margin:0 0 14px">${full ? "Přehled všech part — kdo je s kým, level, XP a příspěvky členů. Řazeno podle all-time crew XP." : "Přehled všech part — kdo je s kým a jaké má role."} (Jen čtení.)</p>
    ${list.map((c) => `
      <div class="panel acrew-card">
        <div class="acrew-head">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div class="faint" style="font-size:12px">${full ? `Lv ${c.level} · ${fmtPts(c.xp)} XP · ` : ""}👥 ${c.member_count}/${c.member_cap}${full ? ` · 🔥 série ${c.streak} (max ${c.best_streak}) · kód ${esc(c.code)}` : ""}</div>
          </div>
        </div>
...
function auctionEditForm(a) {
  openModal(`<div class="modal-body">
    <h2>✏️ Upravit aukci #${a.id}</h2>
    <form class="form" data-submit="save-auction" style="margin-top:16px">
      <input type="hidden" id="aue_id" value="${a.id}">
      <div class="field"><label>Název skinu</label><input class="input" id="aue_name" value="${esc(a.title)}" required></div>
      <div class="field"><label>Obrázek (URL, volitelné)</label>
        <div style="display:flex;gap:8px">
          <input class="input" id="aue_image" value="${esc(a.image_url)}" placeholder="https://… / 📁 z PC / 🔍 skin" style="flex:1">
          <button type="button" class="btn btn-sm" data-action="upload-image" data-prefix="aue" title="Nahrát obrázek z počítače" style="white-space:nowrap">📁 Z PC</button>
          <button type="button" class="btn btn-sm" data-action="skin-picker" data-prefix="aue" title="Vyhledat CS2 skin z katalogu (s náhledy)" style="white-space:nowrap">🔍 Najít skin</button>
        </div>
      ...
      <div class="field-row">
        <div class="field"><label>Vyvolávací cena${a.bids_count ? " 🔒" : ""}</label>
          <input class="input" id="aue_start" type="number" min="1" value="${a.start_bid}" ${a.bids_count ? "disabled title='Už se přihazovalo – nejde měnit'" : ""}></div>
        <div class="field"><label>Min. příhoz</label><input class="input" id="aue_inc" type="number" min="1" value="${a.min_increment}"></div>
      </div>
      <div class="field-row">
        <div class="field"><label>💎 Kup teď (0 = bez)</label><input class="input" id="aue_buynow" type="number" min="0" value="${a.buy_now || 0}"></div>
        <div class="field"><label>⏳ Nový konec za (min, prázdné = nechat)</label><input class="input" id="aue_min" type="number" min="1" placeholder="nechat"></div>
      </div>
      <div class="field-row" style="margin-top:8px">
        <label class="check"><input type="checkbox" id="aue_subonly" ${a.sub_only ? "checked" : ""}> 💜 Jen pro suby</label>
        <label class="check"><input type="checkbox" id="aue_chat" ${a.chat_announce ? "checked" : ""}> 💬 Hlásit příhozy do chatu</label>
      </div>
      <button class="btn btn-primary btn-block" type="submit" style="margin-top:12px">Uložit změny</button>
    </form></div>`);
}
async function saveAuction() {
  const id = $("#aue_id").value;
  const startEl = $("#aue_start");
  const body = {
    title: $("#aue_name").value.trim(), image_url: $("#aue_image").value.trim(),
    min_increment: parseInt($("#aue_inc").value || "1", 10),
    buy_now: parseInt($("#aue_buynow").value || "0", 10),
    sub_only: $("#aue_subonly").checked, chat_announce: $("#aue_chat").checked,
  };
  if (!startEl.disabled) body.start_bid = parseInt(startEl.value || "1", 10);
  const mins = ($("#aue_min").value || "").trim();
  if (mins) body.minutes = parseInt(mins, 10);
  if (!body.title) { toast("Zadej název.", "error"); return; }
  try {
    await api(`/admin/auctions/${id}`, { method: "PUT", body });
    toast("✏️ Aukce upravena.", "success"); closeModal(); adminAuctions();
    loadShopAuctions(); refreshAuctionBanner();
  } catch (e) { toast(e.message, "error"); }
}
async function adminDeleteAuction(id) {
  if (!confirm("Smazat aukci z historie? Nevratné (smaže i historii příhozů).")) return;
  try { await api(`/admin/auctions/${id}`, { method: "DELETE" }); toast("🗑️ Smazáno.", "info"); adminAuctions(); }
  catch (e) { toast(e.message, "error"); }
}
...
function pageFarm() {
  ...
  view.innerHTML = `<div id="farmBox">${skeletonCards(1)}</div>`;
  loadFarm();
  if (!localStorage.getItem("statek_guide_v1")) setTimeout(() => {
    if (currentRoute() === "statek" && !$("#modalRoot").classList.contains("open")) {
      localStorage.setItem("statek_guide_v1", "1");
      farmGuide();
    }
  }, 250);
}
...
async function loadFarm() {
  ...
  try {
    ...
    const foxSlot = f.fox ? f.fox.slot : -1;
    const ready = f.slots.filter((s) => !s.empty && s.state === "ready" && s.slot !== foxSlot);
    ...
    box.innerHTML = `<div class="stk-wrap">
      ...
      <div class="stk-status" style="border-color:${patron.gifts ? "rgba(185,95,255,.45)" : "rgba(255,183,55,.22)"}">
        <span class="stk-s-l">🎁 <b>Patron sezóny</b> <span class="stk-chip${patron.gifts ? " ok" : ""}">${patron.gifts} tento měsíc</span>
          <span class="faint">${patron.title ? esc(patron.title) : `První gift = odznak Dárce farmy`}</span></span>
        <span class="stk-s-r">${patron.slot_bonus ? `<span class="stk-chip ok">🏠 +1 patron slot</span>` : ""}${patron.fox_guard ? `<span class="stk-chip ok">🦊 ochrana lišky</span>` : ""}${patron.next_at ? `<span class="faint" style="font-size:11px">ještě ${patron.next_at - patron.gifts} do dalšího milníku</span>` : `<span class="faint" style="font-size:11px">max. sezónní titul</span>`}</span>
      </div>
      <div class="stk-status" style="border-color:${turbo.count ? "rgba(255,183,55,.5)" : "rgba(255,183,55,.22)"}">
        <span class="stk-s-l">⚡ <b>Farmářské turbo</b> <span class="stk-chip${turbo.count ? " ok" : ""}">${turbo.count}/${turbo.max} žetonů</span>
          <span class="faint">1 gift = 1 žeton · další cyklus vybraného zvířete běží 2× rychleji</span></span>
        <span class="stk-s-r"><span class="faint" style="font-size:11px">žetony propadnou za 7 dní</span></span>
      </div>
      ${f.fox ? `<div class="stk-status" style="border-color:rgba(201,106,42,.5)">
        <span class="stk-s-l">🦊 <b>Liška na statku!</b> <span class="faint">Číhá na tvůj hotový produkt – zaplať výkupné, nebo o něj přijdeš.</span></span>
        <span class="stk-s-r"><button class="bp-claim" data-action="farm-fox" data-pay="1">💰 Zaplatit ${fmtPts(f.fox.ransom)}</button>
        <button class="btn btn-ghost" data-action="farm-fox" data-pay="0">Nechat jí ho 😞</button></span></div>` : ""}
      ${f.contract ? `<div class="stk-status">
        <span class="stk-s-l">📋 <b>Zakázka dne</b> ${f.contract.items.map((i) => `<span class="stk-chip${i.have >= i.goal ? " ok" : ""}">${i.pico} ${esc(i.product)} ${i.have}/${i.goal}${i.have >= i.goal ? " ✓" : ""}</span>`).join("")}</span>
...
function farmBuy(animal, name, icon, cost) {
  const after = (state.user?.points || 0) - Number(cost);
  openModal(`
    <div class="modal-body cf-buy">
      <div class="cf-row">
        <div class="cf-img"><span>${esc(icon)}</span></div>
        <div class="cf-meta"><div class="cf-name">${esc(name)}</div><div class="cf-cost">−${fmtPts(cost)} 🌾</div></div>
      </div>
      <div class="cf-after"><span>Zůstatek po koupi</span><b>${fmtPts(after)} 🌾</b></div>
      <div class="modal-actions">
        <button class="btn btn-primary btn-block" data-action="farm-buy-confirm" data-animal="${esc(animal)}">✓ Koupit zvíře</button>
        <button class="btn btn-ghost" data-action="close-modal">Zpět</button>
      </div>
    </div>`);
}
async function farmBuyConfirm(animal) {
  try {
...
async function farmTurbo(slot) {
  try {
    const r = await api("/farm/turbo", { method: "POST", body: { slot: parseInt(slot, 10) } });
    if (state.user) state.user.points = r.balance;
    toast(`⚡ Turbo použito! ${esc(r.name)} dokončí tento cyklus 2× rychleji · zbývá ${r.turbo_left}`, "success");
    renderHeader(); loadFarm();
  } catch (e) { toast(e.message, "error"); }
}
async function farmFox(pay) {
  if (!pay && !confirm("Fakt nechat lišce produkt? Zvíře zhladoví a produkt je fuč.")) return;
  try {
    const r = await api("/farm/fox", { method: "POST", body: { pay } });
    if (state.user) state.user.points = r.balance;
...
/* ---------- Denní / týdenní úkoly (questy) – plní i Selskou stezku ---------- */
function pageUkoly() {
  const view = $("#view");
  if (!state.user) {
    view.innerHTML = `
    ...
    box.innerHTML = `
      ...
      ${list.map((u) => `<tr>
        ...
        <td><div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
          ${u.timeout_until ? `<span style="background:#3a2e0a;color:#ffd25a;border:1px solid #6b5512;border-radius:999px;padding:2px 7px;font-size:11px;font-weight:800;white-space:nowrap" title="V timeoutu do ${esc(String(u.timeout_until).slice(0, 16).replace("T", " "))} · Důvod: ${esc(u.timeout_reason || "Neuveden")}">⏳ TIMEOUT</span>` : ""}
...
async function adminSecurity() {
  ...
  try {
    ...
    const thr = ts.threshold_per_min || 1000;
...
function farmGuide() {
  ...
  openModal(`<div class="wg"><div class="wg-head"><img class="wg-mascot" src="/sedlak-cut.png" alt="">
    <div><h2 class="wg-title">Jak funguje <span style="color:var(--accent)">Statek</span>? 🚜</h2><p class="wg-sub">Krátký návod, jak z farmy vytěžit maximum.</p></div></div>
    <div class="wg-steps">${steps.map(([ic, t, d]) => `<div class="wg-step"><span class="wg-ico">${ic}</span><div><b>${t}</b><div class="wg-d">${d}</div></div></div>`).join("")}</div>
    <button class="btn btn-primary btn-block" data-action="close-modal">Jdu farmařit! 🌾</button></div>`, "modal-wg");
}
...
function handleAction(action, el) {
  ...
  switch (action) {
    ...
    case "farm-buy": farmBuy(el.dataset.animal, el.dataset.name, el.dataset.icon, el.dataset.cost); break;
    case "farm-buy-confirm": farmBuyConfirm(el.dataset.animal); break;
    ...
    case "crew-settings": crewSettingsModal(); break;
    case "crew-settings-save-motd": crewSettingsSaveMotd(); break;
    case "crew-history": crewHistoryModal(); break;
    case "crew-war-open": crewWarModal(); break;
    case "crew-war-declare": crewWarDeclare(el.dataset.id); break;
    ...
    case "auction-edit": { const a = (window._adminAucList || []).find((x) => x.id === parseInt(el.dataset.id, 10)); if (a) auctionEditForm(a); break; }
    case "auction-delete": adminDeleteAuction(parseInt(el.dataset.id, 10)); break;
...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => { navigator.serviceWorker.register("/sw.js?v=2026071270").catch(() => {}); });
}
...
/* ---------------- 🤝 Parta / Crew (klany) ---------------- */
let _crewId = null, _crewPoll = null, _crewDetail = null;
...
function _crewIdleBadge(lastActive) {
  if (!lastActive) return `<span class="crew-idle crew-idle-never" title="Nikdy se nepřihlásil(a)">⚪</span>`;
  const days = (Date.now() - new Date(lastActive).getTime()) / 86400000;
  const cls = days < 1 ? "crew-idle-active" : days < 7 ? "crew-idle-recent" : days < 14 ? "crew-idle-warn" : "crew-idle-dead";
  const dot = days < 1 ? "🟢" : days < 7 ? "🟡" : days < 14 ? "🟠" : "🔴";
  return `<span class="crew-idle ${cls}" title="Naposledy aktivní: ${timeAgo(lastActive)}">${dot}</span>`;
}
function crewMembersHTML(d) {
  return (d.members || []).map((m) => {
    ...
    <div class="crew-mrow${m.is_you ? " you" : ""}">
      ...
      <span class="crew-mname">${_crewRole(m.role)}${esc(m.username)}${m.is_you ? ' · <span style="color:var(--accent)">TY</span>' : ""}${d.is_leader && !m.is_you ? " " + _crewIdleBadge(m.last_active) : ""}</span>
...
function crewWarHTML(d) {
  ...
  const cooldown = Number(d.war_cooldown_seconds) || 0;
  ...
  if (!d.war) {
    ...
    return `<div class="panel crew-war-panel" style="margin-bottom:14px">
      ...
      ${canWar
        ? `<div class="faint" style="font-size:13px;margin-bottom:8px">${cooldown ? `⏳ Tvoje parta má cooldown ještě <b>${grdDur(cooldown)}</b>.` : "Vyhlas válku jiné partě — 1 den, kdo nasbírá víc XP vyhrává. Pak mají obě party 2 dny pauzu. Odměna = sláva, žádné sedláky."}</div>
           <button class="btn btn-sm ${cooldown ? "btn-ghost" : "btn-accent"}" data-action="crew-war-open" ${cooldown ? "disabled" : ""}>${cooldown ? `⏳ Cooldown ${grdDur(cooldown)}` : "⚔️ Vyhlásit válku"}</button>`
        : `<div class="faint" style="font-size:13px">Zatím žádná aktivní válka.</div>`}
    </div>`;
  }
  const w = d.war;
  const total = Math.max(1, w.my_gain + w.opp_gain);
  const myPct = Math.round(w.my_gain / total * 100);
  const leading = w.my_gain > w.opp_gain;
  const tied = w.my_gain === w.opp_gain;
  return `<div class="panel crew-war-panel crew-war-active" style="margin-bottom:14px">
    <div class="section-title" style="margin-top:0">⚔️ VÁLKA proti ${w.opponent_emblem} <b>${esc(w.opponent_name)}</b>${w.opponent_tag ? ` <span class="crew-tag">[${esc(w.opponent_tag)}]</span>` : ""}</div>
    <div class="crew-war-bar"><div class="crew-war-fill${leading ? " leading" : tied ? "" : " losing"}" style="width:${myPct}%"></div></div>
    <div class="crew-war-nums"><span><b>${Number(w.my_gain).toLocaleString("cs-CZ")}</b> my</span><span class="faint">vs</span><span><b>${Number(w.opp_gain).toLocaleString("cs-CZ")}</b> oni</span></div>
    <div class="faint" style="font-size:12px;margin-top:6px">⏳ Válka končí za ${countdown(w.ends_at)}</div>
  </div>`;
}
...
async function crewPoll() {
  ...
  const wb = document.getElementById("crewWar"); if (wb) wb.innerHTML = crewWarHTML(d);
...
async function crewSettingsModal() {
  if (!_crewId || !_crewDetail || !_crewDetail.is_leader) return;
  const d = _crewDetail;
  let emData; try { emData = await api("/crews/emblems"); } catch (e) { emData = { emblems: [], cost: 0 }; }
  const emGrid = (emData.emblems || []).map((em) => `<button class="crew-em-pick${em === d.emblem ? " sel" : ""}" data-action="crew-set-emblem" data-emblem="${em}">${em}</button>`).join("");
  openModal(`
    <h3 style="margin-top:0">⚙️ Nastavení party</h3>
    <div class="field" style="margin-bottom:16px">
      <label>📝 Popis party (MOTD)</label>
      <textarea class="input" id="crewMotdInput" maxlength="200" rows="3" style="width:100%;resize:vertical;margin-top:6px">${esc(d.motd || "")}</textarea>
      <button class="btn btn-sm btn-accent" style="margin-top:8px" data-action="crew-settings-save-motd">Uložit popis</button>
    </div>
    <div class="field" style="margin-bottom:16px">
      <label>🔒 Soukromí</label>
      <p class="faint" style="font-size:12.5px;margin:4px 0 8px">${d.private ? "Parta je soukromá — nový člen musí počkat na schválení." : "Parta je veřejná — kdokoliv s kódem se přidá rovnou."}</p>
      <button class="btn btn-sm btn-ghost" data-action="crew-private" data-private="${d.private ? 0 : 1}">${d.private ? "🔓 Zveřejnit partu" : "🔒 Udělat soukromou"}</button>
    </div>
    <div class="field">
      <label>🎨 Emblém <span class="faint" style="font-weight:400">· změna ${fmtPts(emData.cost)}</span></label>
      <div class="crew-em-grid" style="margin-top:8px">${emGrid}</div>
    </div>`);
}
async function crewSettingsSaveMotd() {
  if (!_crewId) return;
  const el = document.getElementById("crewMotdInput");
  const text = el ? el.value : "";
  try { const d = await api("/crews/" + _crewId + "/motd", { method: "POST", body: { text } }); closeModal(); toast("Popis uložen.", "success"); renderCrewDetail(d); }
  catch (e) { toast(e.message, "error"); }
}
async function crewPrivate(p) {
  if (!_crewId) return;
  try { const d = await api("/crews/" + _crewId + "/private", { method: "POST", body: { private: String(p) === "1" } }); closeModal(); toast(d.private ? "Parta je teď soukromá 🔒" : "Parta je veřejná 🔓", "success"); renderCrewDetail(d); }
  catch (e) { toast(e.message, "error"); }
}
...
async function crewHistoryModal() {
  if (!_crewId) return;
  let data;
  try { data = await api("/crews/" + _crewId + "/log"); } catch (e) { toast(e.message, "error"); return; }
  const rows = (data.events || []).map((e) => `
    <div class="crew-log-row">
      <span class="crew-log-text">${e.actor_name ? `<b>${esc(e.actor_name)}</b> ` : ""}${esc(e.verb)}${e.target_name ? ` <b>${esc(e.target_name)}</b>` : ""}${e.detail ? ` <span class="faint">— ${esc(e.detail)}</span>` : ""}</span>
      <span class="crew-log-time faint">${timeAgo(e.created_at)}</span>
    </div>`).join("") || '<div class="faint">Zatím žádná historie.</div>';
  openModal(`<h3 style="margin-top:0">📜 Historie party</h3><div class="crew-log-list">${rows}</div>`, "modal-lg");
}
async function crewWarModal() {
  ...
  const candidates = (lb.crews || []).filter((c) => c.id !== _crewId).slice(0, 30);
  const rows = candidates.map((c) => {
    const cooldown = Number(c.war_cooldown_seconds) || 0;
    const action = c.at_war
      ? '<button class="btn btn-sm btn-ghost" disabled>⚔️ Ve válce</button>'
      : cooldown
        ? `<button class="btn btn-sm btn-ghost" disabled title="Cooldown po válce">⏳ ${grdDur(cooldown)}</button>`
        : `<button class="btn btn-sm btn-accent" data-action="crew-war-declare" data-id="${c.id}">⚔️ Vyhlásit</button>`;
    return `
    <div class="crew-war-pick-row">
      <span class="crew-emblem">${c.emblem}</span>
      <span class="crew-name"><b>${esc(c.name)}</b> ${c.tag ? `<span class="crew-tag">[${esc(c.tag)}]</span>` : ""} <span class="faint" style="font-size:11.5px">⭐${c.level} · ${c.members}👤</span></span>
      ${action}
    </div>`;
  }).join("") || '<div class="faint">Žádná jiná parta zatím nečeká na výzvu.</div>';
  openModal(`<h3 style="margin-top:0">⚔️ Vyhlásit válku</h3><p class="faint" style="margin-top:-4px;font-size:13px">1 den, kdo nasbírá víc crew XP vyhrává. Poté mají obě party 2 dny pauzu. Odměna = sláva (status), žádné sedláky.</p><div class="crew-war-pick-list">${rows}</div>`, "modal-lg");
}
async function crewWarDeclare(id) {
  if (!_crewId || !id) return;
  try {
    const d = await api("/crews/" + _crewId + "/war/declare", { method: "POST", body: { opponent_id: parseInt(id, 10) } });
    closeModal(); toast("⚔️ Válka vyhlášena!", "success"); renderCrewDetail(d);
  } catch (e) { toast(e.message, "error"); }
}
